...
from agents.ddqn import DDQNParams, Basic2dEncoder, QModel, Transition, TRANSITION_TUPLE_SIZE
...
class SafetyDDQNParams(DDQNParams):
  def __init__(self, spec_encoding_hidden_size=100, **kwargs):
...
class SafetyDDQNAgent(Agent):
  ...
  def _policy_net_pass(self, state: SafetyState):
    encoded_formula = torch.mean(self._spec_encoder(state.formula), dim=1)
  ...
  def train(self, env: SafetyConstrainedEnv, print_logging=True, writer=None):
    ...
    for i_episode in range(self.params.episodes):
      ...
      one_hot_safety_spec = get_one_hot_spec(safety_spec_strs, env.get_num_props()).unsqueeze(0)
