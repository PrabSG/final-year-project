"""Models for building Latent Shielded Dreamer Agent.
...
class ShieldedActorModel(jit.ScriptModule):
  def __init__(self, belief_size, state_size, hidden_size, action_size, shield, dist='tanh_normal',
                activation_function='elu', min_std=1e-4, init_std=5, mean_scale=5):
    super().__init__()
    self.act_fn = getattr(F, activation_function)
    self.fc1 = nn.Linear(belief_size + state_size, hidden_size)
  ...
  def get_action(self, belief, state, det=False):
    ...
    dist = SampleDist(dist)
    if det: 
      return self._shield.step(dist.mode())
    else: 
      return self._shield.step(dist.rsample())
